...
<c:set var="pageTitle" value="Trang quản lý" />
		...
		<c:forEach var="c" items="${listcate}">
			<tr>
				...
				<td><c:if test="${not empty c.images}">
						<img src="${c.images}" alt="Category Image"
							style="max-width: 100px; max-height: 100px;">
					</c:if></td>
				<td>${c.user.username}</td>
				<td><a
					href="${pageContext.request.contextPath}/manager/category/edit?id=${c.id}"
					class="btn btn-sm btn-warning">Sửa</a>
					<form
						action="${pageContext.request.contextPath}/manager/category/delete"
